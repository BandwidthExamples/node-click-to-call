...
router.get('/buttons/:id/calls', authenticated, async ctx => {
	...
	const query = {_id: objectId(ctx.params.id), user: objectId(ctx.state.user.id)};
	ctx.body = {
		page,
...
router.options('/buttons/:id/click', async ctx => {
	debug('OPTIONS /buttons/:id/click');
	ctx.headers['Access-Control-Allow-Origin'] = ctx.request.origin;
	ctx.headers['Access-Control-Allow-Methods'] = 'POST, OPTIONS';
...
router.get('/buttons/sse', async ctx => {
	...
	redis.on('message', (channel, message) => stream.push(JSON.stringify(message)));
