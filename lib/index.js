		...
		.use(middlewares.koa({
			...
			callCallback: async (data, ctx) => {
				if (data.eventType === 'answer') {
					...
					const tracking = CallTracking.findById(objectId(trackingId)).populate('button');
					if (!tracking || tracking.button.number !== data.to) {
						return;
					}
					let user = null;
					...
					if (user) {
						...
						await ctx.bandwidthApi.Call.playAudioAdvanced(data.callId, {
							fileUrl: `${ctx.protocol}://${ctx.host}/public/ring.wav`,
							loopEnabled: true
						});
