...
async function main() {
	...
	const domainName = `c${crypto.createHash('md5').update(BANDWIDTH_USER_ID + BANDWIDTH_API_TOKEN + BANDWIDTH_API_SECRET).digest('hex').substr(14)}`;
	app
			...
			callCallback: async (data, ctx) => {
				if(data.eventType === 'answer'){
					...
					if (user) {
						...
						const call = await bandwidthApi.Call.get(data.callId);
						...
						await ctx.bandwidthApi.Call.playAudioAdvanced(anotherCallId, {fileUrl: ''}); // stop rings
					}
				}
