...
UserSchema.statics.checkUser = async (userName, password) => {
	const user = await User.findOne({email: userName, emailConfirmed: true});
...
ResetPasswordRequest = mongoose.models.ResetPasswordRequest || mongoose.model('ResetPasswordRequest', ResetPasswordRequestSchema);
...
ConfirmEmailRequest = mongoose.models.ConfirmEmailRequest || mongoose.model('ConfirmEmailRequest', ConfirmEmailRequestSchema);
...
function callTrackingPostSave(doc) {
	redis.publish(`CallTracking.${(doc.user.id || doc.user).toString()}`, JSON.stringify(doc.toJSON()));
}

CallTrackingSchema.post('save', callTrackingPostSave);

module.exports = {
	User,
	ResetPasswordRequest,
	ConfirmEmailRequest,
	Button: mongoose.models.Button || mongoose.model('Button', ButtonSchema),
	CallTracking: mongoose.models.CallTracking || mongoose.model('CallTracking', CallTrackingSchema),
	callTrackingPostSave
};
